...
- [[活動_貿聯-KY_2026Q2法說_20260821]] 表示高電流／高電壓供電架構會依客戶與部署條件並存，貿聯不押單一規格；此支持 400V／800V 與既有架構並行的判斷，不能視為 800VDC 已全面量產。
- [[活動_廣閎科_2026Q2法說_20260821]] 提到 AI rack 功率由約 120kW 向 250–440kW、600kW–1MW 的產業情境，以及 BPU／SiC 高壓 MOS 方向；數字與時程為法說整理，低至中信心，需以正式資料驗證。

## 2026-09-15 UBS：400V／800V 與 HVDC 採用節奏

- 台達電管理層表示 HVDC 已開始生產與出貨，2027 年先小量，後續 1–2 年再加速；三家 hyperscaler 評估 400V，其中兩家先用於 NVIDIA 機櫃，後續可能收斂到 800V。這是管理層／券商展望，不等同全面量產。
- 旭隼 800VDC power shelf 預計 2026 年底送樣、2027H1 pilot run、2027H2 量產；此為產品驗證與製造節點，與完整資料中心 800VDC 架構大規模部署需分開追蹤。
- 上述資訊支持「400V HVDC 與 800VDC 並行、分階段導入」的判斷，亦與 Rubin 世代非必然需要完整 800V 的既有衝突資訊並列。

來源：[[報告_UBS_台達電_20260915]]、[[報告_UBS_旭隼_20260915]]（UBS，2026-09-15）。

## Citi 2026-09-20 更新：按機房條件分階段導入

- Citi 認為 800VDC 的系統價值明確，但不必然是 Rubin 或每一個 Rubin Ultra 部署的必要條件；新建資料中心與 neocloud 可從電力基礎設施一開始整合高壓直流，既有機房則可能延續替代架構。
- 因此短期應同時追蹤 sidecar power shelf、400V／800V 方案、power-management IC、電容與熱管理，而非只用單一 800VDC 滲透率判斷電源供應鏈景氣。
