---
title: "CHKP.US(check point software)"
ticker: "CHKP"
market: US
exchange: NASDAQ
sector: Cybersecurity / Network Security / AI Security
tags:
  - 公司/Check Point
  - 產業/資安
  - 環節/硬體安全設備
  - 主題/NGFW
  - 主題/SASE
  - 主題/AI驅動資安
updated: 2026-08-24
aliases:
  - Check Point Software Technologies
  - Check Point
  - CHKP
related_companies:
  - "[[PANW.US(palo alto networks)]]"
  - "[[FTNT.US(fortinet)]]"
  - "[[CRWD.US(crowdstrike)]]"
  - "[[ZS.US(zscaler)]]"
---

# CHKP.US(check point software)

## 基本資料

Check Point Software Technologies 是以網路安全為核心的以色列軟體與整合式硬體安全廠商，產品涵蓋防火牆設備、軟體 blades、雲端與訂閱服務，服務超過 100,000 個組織。公司以 FireWall-1 與 Stateful Inspection 起家，現以 Infinity Platform、ThreatCloud AI 與集中式管理，延伸至 Hybrid Mesh Network Security、Workspace Security、Exposure Management 與 AI Security。

2026Q2 營收為 $673.6M、年增 1.3%；Security Subscriptions 為 $332.6M、年增 11.6%，但 Products & Licenses 為 $113.4M、年減 14.0%，反映防火牆更新週期與 GTM 重組造成的大型交易遞延。主要銷售透過經銷商、VAR、SI 與 MSP，2026Q2 美洲與 EMEA 各約佔 44% 營收、APAC 約 12%。資料來自 2026-07-30 至 2026-08-14 券商報告。

公司 2026-07-30 財報簡報確認：Q2 營收 $674M、訂閱收入 $333M（年增 12%）、Non-GAAP EPS $2.55（年增 8%）、調整後 FCF $161M；Deferred Revenue 為 $2.025B、RPO 為 $2.552B，均年增 7%。Q2 包含約 $28M 以色列研發補助利益，公司另回購約 250 萬股、金額約 $325M。上述為 company disclosure / fact，與券商估計分開保留。

## 核心技術／競爭優勢

- **Stateful Inspection 與軟體 blades**：以連線狀態判斷流量並在同一管理架構中組合多種安全功能，支撐既有防火牆裝機基礎。
- **Infinity Platform**：將網路、工作空間、暴露面管理與 AI 安全整合至共享威脅情報與集中管理，降低多產品治理複雜度。
- **AI Defense Plane／AI Network Firewall**：提供 AI prompt、agent 與 model interaction 的可視性、治理與防護，並可附加在既有 appliance，不一定需要新購設備。
- **AI Network Firewall 正式發布**：公司在 2Q26 法說同步推出 AI Network Firewall，將 AI Defense Plane 延伸至網路、私有／公有雲與 SASE，主打發現 AI app／agent、控管存取、依 prompt use case 保護敏感資料與阻擋 AI app 威脅；目前仍屬產品發布與需求驗證，尚非已確認的獨立營收驅動。
- **訂閱與新興產品組合**：Email Security、CTEM 與 AI Security 2026Q2 合計 ARR 年增逾 40%、calculated billings 年增逾 35%；訂閱收入比重提升，改善收入可見度。

## 產品與應用

| 產品／服務 | 應用 | 相關客戶／下游 |
|---|---|---|
| Quantum／網路安全設備 | 企業、資料中心與混合網路防火牆 | 企業、政府、電信商 |
| Infinity Platform／Hybrid Mesh | 多雲、分支據點、跨網路集中治理 | [[PANW.US(palo alto networks)]]、[[FTNT.US(fortinet)]] 為競爭參考 |
| Email Security／CTEM | 郵件防護、暴露面驗證與漏洞優先排序 | 企業 SOC、SI／MSP 通路 |
| AI Network Firewall／AI Security | AI agent、LLM 互動與 AI 應用的提示注入、資料外洩與治理 | AI 導入企業 |

## 圖片 / 架構圖

![[2026-08-14-CHKP.OQ-Zacks Equity Researc-Check Point Software Technologies Ltd(CHKP) Zacks Company Re...-123866992_003.png]]
*Zacks（2026-08-14）：Check Point 股價與 12 個月 EPS 共識預估走勢；圖中 2026E–2027E EPS 預估仍受 2026 年轉型期壓力，供追蹤估值修復。*

![[CHKP 260721_2Q26 preview WF_003.png]]
*Wells Fargo（2026-07-21）：收入組合、管理層季度指引與 billings／RPO 預估，顯示 2026Q3 低谷後，模型將主要回升壓在 2026Q4。*

![[CHKP_2Q26_Earnings_Presentation_page4.png]]
*Check Point（2026-07-30）：AI Network Firewall 將員工、AI agents、應用與本地工具的流量，集中至 perimeter／data center／cloud firewall，並對 LLM、AI chats、Gen AI apps 與 public tools 提供可視性、存取控制、資料保護與威脅阻擋。*

## EPS 記錄

| 季度 | Non-GAAP EPS | 營收 | 備註 |
|---|---:|---:|---|
| 2026Q2 | $2.55 | $673.6M | EPS 高於市場預期；營收年增 1.3% |

## EPS 預估

| 年度 | TD Cowen（2026-07-13） | Wells Fargo（2026-07-21） | Barclays（2026-07-30） | BofA（2026-07-30） | JPMorgan（2026-07-30） | Zacks（2026-08-14） |
|---|---:|---:|---:|---:|---:|---:|
| 2026E | $10.55 | — | $10.45 | $10.39 | $10.63 | $10.45 |
| 2027E | $11.55 | — | $11.40 | $11.37 | $11.71 | $11.30 |

## 目標價與評等

| 券商 | 報告日 | 評等 | 目標價 | 評價基礎 |
|---|---|---|---:|---|
| Deutsche Bank | 2026-07-30 | Hold | $130 | DCF |
| J.P. Morgan | 2026-07-30 | Neutral | $135 | 7x CY27E FCF |
| Barclays | 2026-07-30 | Equal Weight | $140 | 約 11x FY27E FCF |
| BofA | 2026-07-30 | Neutral | $140 | 約 10x 2027E EV/FCF |
| Jefferies | 2026-07-30 | Buy | $150 | 11x EV/2027E FCF |
| Zacks | 2026-08-14 | Neutral | $141 | 4.94x forward 12M sales |
| TD Cowen | 2026-07-13 | Buy | $180 | 平台策略與 2H26／FY27 回升 |
| Wells Fargo | 2026-07-21 | Equal Weight | $140 | 約 11x forward NTM estimates |

## 時間軸

| 時間 | 事件 | 類型 | 重要性 | 備註 |
|---|---|---|---|---|
| 2026Q2 | 2Q 營收 $673.6M、訂閱年增 11.6%、產品年減 14% | 財報 | ⭐⭐⭐ | [[2026-07-30-CHKP.OQ-Barclays-Check Point Software Technologies Ltd. 4Q Ramp Is Main Push...-123496484]] |
| 2026Q3 | 管理層預期產品收入仍年減、業務處於 trough | 財報／低谷 | ⭐⭐⭐ | 3Q guidance revenue $655–685M、訂閱 $332–343M；Wells Fargo 估產品年減 17% |
| 2026Q4 | 大型 appliance deal 與產品收入預期回升 | 放量 | ⭐⭐⭐ | 高度後置，需觀察 close rate 與 pipeline |
| 2026H2 | 約 300 名 quota-bearing reps 招募，其中約 100–150 名淨新增 | 組織 | ⭐⭐ | 主要效益預期自 2027 年顯現，可能先壓 FY27 margin |
| 2026H2 | AI Network Firewall、價格調整與新興產品 pipeline | 新產品／價格 | ⭐⭐ | 未納入部分券商 FY26 基準估計 |
| 2026H2 | 3Q 可能再調價以抵銷記憶體成本 | 價格 | ⭐⭐ | J.P. Morgan；價格上調與 AI Network Firewall 尚未納入其基準展望 |
| 2026-07-30 | 公司發布 AI Network Firewall，並將 AI Defense Plane 延伸至網路、雲端與 SASE | 新產品 | ⭐⭐⭐ | 公司法說簡報；產品 traction 與可辨識 ARR／billings 仍待追蹤 |
| 2027Q1–Q2 | 新增數百名 salespeople 的銷售生產力開始反映 | 組織／放量 | ⭐⭐⭐ | 公司法說；2026 指引未納入招募 ramp，效益預期自 2027 年出現 |

→ 跨公司催化劑詳見 [[時程_2026Q3Q4_AI網通與硬體催化劑]]。

## 供應鏈位置

- **安全平台與硬體設備**：位於企業網路安全軟體、NGFW appliance 與訂閱服務環節。
- **通路**：主要透過 distributor、reseller、VAR、SI 與 MSP 銷售；本批資料未揭露具名供應商或客戶。
- **相關主題**：[[技術_SASE]]、[[技術_EDR與XDR]]、[[分析_AI驅動資安支出2026]]。

## 相關公司

| 公司 | 關係 | 說明 |
|---|---|---|
| [[PANW.US(palo alto networks)]] | 競爭者 | 平台化、SASE 與企業網路安全競爭；J.P. Morgan partner checks 指 CHKP 可能面臨較大型平台的防火牆份額壓力。 |
| [[FTNT.US(fortinet)]] | 競爭者 | NGFW／SASE 與硬體價格調整的比較對象。 |
| [[CRWD.US(crowdstrike)]] | 競爭者 | 雲原生端點與 AI 資安平台，代表不同的資安平台路線。 |
| [[ZS.US(zscaler)]] | 競爭者 | 雲端 SASE／SSE 競爭者；CHKP 的 SASE 技術與整合廣度仍被券商追蹤。 |

> [!warning] 風險與注意事項
> - **執行風險**：2Q 大型 appliance deals 延至 4Q，FY26 指引因此高度後置；若 4Q close rate 未改善，營收與評價可能再次下修。
> - **產品週期與競爭**：產品收入年減 14%、維護與服務收入年減，且券商 partner checks 指向大型平台競爭與 SASE 廣度壓力。
> - **利潤風險**：記憶體成本、AI 投資與新增銷售人力可能在新營收貢獻前壓低毛利或營業利益率。
> - **上行仍待驗證**：AI Network Firewall、新興產品 ARR 成長與可能的價格調整尚不足以抵銷傳統 appliance 弱勢，需觀察 2026Q4／2027 是否轉化為收入。

## 來源

- [[2026-07-30-CHKP.OQ-Barclays-Check Point Software Technologies Ltd. 4Q Ramp Is Main Push...-123496484]] — Barclays，2026-07-30
- [[2026-07-30-CHKP.OQ-BofA Global Research-Check Point Software Technologies Big pipeline energy, smal...-123498847]] — BofA，2026-07-30
- [[2026-07-30-CHKP.OQ-Deutsche Bank-Check Point 2Q - A business still in transition-123496190]] — Deutsche Bank，2026-07-30
- [[2026-07-30-CHKP.OQ-Jefferies-GTM Weakness Strikes Again; Improvement Delayed Til 4Q-123498707]] — Jefferies，2026-07-30
- [[2026-07-30-CHKP.OQ-JPMorgan-Check Point Software GTM Disruption Persists as Unchanged FY...-123497989]] — J.P. Morgan，2026-07-30
- [[2026-08-03-CHKP.OQ-Barclays-Check Point Software Technologies Ltd. Takeaways from Our M...-123591707]] — Barclays 管理層會議，2026-08-03
- [[2026-08-06-CHKP.OQ-Zacks Equity Researc-Check Point Software Technologies Ltd(CHKP) Zacks Company Re...-123707143]] — Zacks，2026-08-06
- [[2026-08-14-CHKP.OQ-Zacks Equity Researc-Check Point Software Technologies Ltd(CHKP) Zacks Company Re...-123866992]] — Zacks，2026-08-14
- [[CHKP 260713_2Q26 preview TD]] — TD Cowen，2026-07-13
- [[CHKP 260721_2Q26 preview WF]] — Wells Fargo Securities，2026-07-21
- [[CHKP 260730_2Q26 results Barclays]] — Barclays，2026-07-30
- [[CHKP 260730_2Q26 results JP]] — J.P. Morgan，2026-07-30
- [[CHKP 260730_2Q26 results TD]] — TD Cowen，2026-07-30
- [[CHKP_2Q26_Earnings_Presentation]] — Check Point Software Technologies，2026-07-30
- [[Check Point Software Technologies Ltd Earnings Call 2026730 DN000000003114523854 (1)]] — Bloomberg transcript，2026-07-30

- [[2026-07-31-CHKP.OQ-Morningstar, Inc.-Morningstar  Check Point Earnings Next-Quarter Guidance Di...-123517419]]（2026-07-31）
- [[Cybersec 260707 Barclays_Security VaR Call]]（2026-07-07）
- [[Cybersec 260710 Barclays_Another Security VaR Call]]（2026-07-10）
- [[Cybersec 260720 WF_2Q26 on-cycle security reseller survey]]（2026-07-20）

## 相關頁面

- [[分析_Check Point_GTM轉型與AI資安2026]]
- [[技術_SASE]]
- [[技術_EDR與XDR]]
- [[分析_AI驅動資安支出2026]]
