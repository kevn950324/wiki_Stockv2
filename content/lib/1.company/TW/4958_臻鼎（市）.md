...
|------|----------|-----|------|
| 2023A | NT$（原始數值待核對）.52 | — | — |
| 2024A | NT$（原始數值待核對）.04 | +100% | — |
| 2025E | NT$（原始數值待核對）.68 | -15% | Citi 估；消費電子需求偏弱 |
| 1Q26E（Citi） | NT$（原始數值待核對）.89 | +35% YoY | 季節性低點；4Q25 實績 NT$（原始數值待核對）.82 |

## EPS 預估

| 年度 | Citi（2026-04-22） | GS 3/20（2026-03-20） | GS 4/18（2026-04-18） | 備註 |
|------|------------------|----------------------|----------------------|------|
| 2026E | NT$（原始數值待核對）.97 | NT$（原始數值待核對）.34 | NT$（原始數值待核對）.45 | GS 因 ABF/PCB 漲價上修；Citi 下修（CCL 缺料風險） |
| 2027E | NT$（原始數值待核對）.80 | NT$（原始數值待核對）.61 | NT$（原始數值待核對）.04 | 差異極大：GS 假設 ABF 基板貢獻大幅成長 |
| 2028E | NT$（原始數值待核對）.98 | NT$（原始數值待核對）.56 | NT$（原始數值待核對）.25 | — |

> [!warning] 臻鼎評等重大衝突：Citi Sell vs GS Buy
>
> **Citi（2026-04-22）Sell，TP NT$（原始數值待核對）**
> - 核心論點：消費電子曝險 80-90%（Apple）、激進資本支出（NT$（原始數值待核對）/500/500 億 for 2026/27/28）、CCL 缺料風險（Tier 2 PCB 廠優先順位低）
> - ZDT ABF 基板佔比約 4-5%，不足以抵銷消費電子下行壓力
> - 高折舊負擔（2026E 折舊 NT$（原始數值待核對） 億）壓縮毛利率
> - TP NT$（原始數值待核對） = 13x 2027E EPS NT$（原始數值待核對）.80
>
> **GS（2026-04-18）Buy，TP NT$（原始數值待核對）**
...
> - TP NT$（原始數值待核對） = SOTP（ABF+BT 基板 2.6x P/B + AI PCB 20x P/E + 消費電子 PCB 8x P/E）
>
> **分歧根源**：對 ZDT ABF 基板規模與 Rubin 進入時程的假設差異極大。GS 2027E EPS NT$（原始數值待核對）.04 是 Citi NT$（原始數值待核對）.80 的 2.2 倍。

## 目標價與評等

| 券商 | 報告日 | 評等 | 目標價 | 評價基礎 | 來源 |
|------|--------|------|--------|----------|------|
| Citi | 2026-04-22 | **Sell** | NT$（原始數值待核對） | 13x 2027E EPS NT$（原始數值待核對）.80（DCF based） | [[報告_Citi_PCB_CCL_20260422]] |
| Goldman Sachs | 2026-03-20 | **Buy** | NT$（原始數值待核對） | SOTP：ABF 2.6x P/B + AI PCB 20x P/E + 消費 8x P/E | [[報告_GS_CCL_PCB_20260320]] |
| Goldman Sachs | 2026-04-18 | **Buy** | NT$（原始數值待核對） | SOTP（同上，滾動期間調整+EPS 上修） | [[報告_GS_台灣CCL_PCB_20260418]] |
| Nomura | 2026-06-30 | **Buy**（維持） | NT$720（舊 NT$510） | 新興 AI PCB/HDI 廠；Nvidia/Google/AWS 滲透加速 2H26F | [[報告_Nomura_亞洲AI半導體_20260630]] |

## 時間軸

| 時間 | 事件 | 類型 | 重要性 | 備註 |
|------|------|------|--------|------|
| 2026-03-20 | GS 上調 TP NT$（原始數值待核對）→NT$（原始數值待核對） | 評等調整 | ⭐⭐ | ABF 基板利用率改善 |
| 2026-04-18 | GS 再上調 TP NT$（原始數值待核對）→NT$（原始數值待核對），+30% | 大幅上調 | ⭐⭐⭐ | Rubin 計算板進入、TPU v8 候選 |
| 2026-04-22 | Citi 維持 Sell，上調 TP NT$（原始數值待核對）→NT$（原始數值待核對） | 維持賣出 | ⭐⭐⭐ | 消費電子曝險+激進資本支出論點 |
...
### 2026-08-23 AI PCB 更新

- [[260812_gs_ZDT]]：AI HDI／多層 PCB 由 2H26 起放量，主流 AI PCB 層數約 20–30 層，新案可能提升至 30–50／70–80 層；泰國與淮安廠良率屬公司揭露，仍需持續驗證。
- Goldman Sachs 目標價 NT$925，長期擴產延伸至 2030 年；量產與市占屬 estimate，不視為已確認訂單。
