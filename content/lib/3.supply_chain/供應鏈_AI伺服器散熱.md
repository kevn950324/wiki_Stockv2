...
flowchart TD
    ...
    RACK["機架結構<br/>川湖 2059（滑軌）<br/>勤誠 8210（機殼／機櫃）"]
...
| 廠商 | 地位 | 備註 |
|------|------|------|
| [[2421_建準（市）]] | 風扇與散熱模組供應商 | Daiwa 2026-08-28 估 2026 年底向 ODM／企業客戶交付伺服器風扇模組；EC 風扇、電力機櫃氣冷與冷板／CDU 液冷產品同步拓展 |
...
| 廠商 | 地位 | 備註 |
|------|------|------|
| [[2059_川湖（市）]] | 全球伺服器滑軌龍頭 | NVL72 單組機櫃需 70+ 種滑軌品項；中信將 GB300／VR200 滑軌 ASP 上修至 US$200／US$300–350；AMD Helios 滑軌已完成認證 |
...
2. **MCL 導入時程**：較新供應鏈調查把基準時程延至 2028H2 Feynman；需追蹤良率與供應鏈準備度。Rubin Ultra 先採約 US$200 的可拆式均熱片，仍可推升健策 ASP，但對雙鴻／奇鋐水冷板的替代衝擊後移。
...
7. **ASIC 水冷與機構件放量**：J.P. Morgan 2026-07-23 估 [[3017_奇鋐（市）]] 的 AWS Trainium 3／Google TPU v8 cold plate 於 4Q26 放量；中信 2026-08 另估 2027 年水冷滲透率突破 50%，並指 AWS T3 水冷櫃於 4Q26 起出貨。直接觀察奇鋐液冷產品組合與 [[8210_勤誠（市）]] 的 PDS／Chassis 出貨回補。
8. **系統整合端液冷產能**：[[SMCI.US(supermicro)]] 的月產能口徑顯示液冷已占總 rack capacity 至少一半；實際營收確認仍取決於客戶電力、冷卻水路與機房完工，需把「產能」與「已出貨」分開追蹤。
9. **VPD 的雙面冷卻門檻**：[[技術_垂直供電VPD]] 將 VRM／功率元件移至載板背面，冷板必須同時兼顧雙面流道、壓降、平整度、鎖附應力與防漏水；中信投顧 2026-09-17 認為雙面冷板已達 4kW+ 熱負載、朝 6kW 演進，ASP 約為單面方案 1.5–2 倍。[[3017_奇鋐（市）]]、[[3324_雙鴻（市）]]的受惠仍須以客戶採用與良率驗證。
10. **短中期 lid 路線未收斂**：Rubin Ultra 同時驗證兩片式與 Removable Lid；後者可在 ODM 組裝時重建 TIM1.5 與冷板介面，被中信視為短中期較具量產機會。MCL 仍有微流道、密封、漏液與可靠度門檻，現階段不應把它當成已定案的水冷板替代。
...
- 延伸服務整合：[[2480_敦陽科（市）]]（AI 算力、雲端遷移與地端推理導入；非散熱零組件供應商）
- 控制器方案延伸：[[7750_新代（市）]]（AI 伺服器液冷設備加工控制方案；非液冷零組件供應商）
...
## 2026-09-08 GFHK Rubin 散熱模型更新

- GFHK 將 GPU TDP 路徑由 Hopper／Ampere 約 400–700W、Blackwell 約 1,000–1,200W，推演至 Rubin 最高約 2,300W；GB300 機架約 130–140kW，未來世代可能走向 600kW–1MW，屬券商產業模型。
- VR200 NVL72 冷板內容值估約 US$34,200／rack，QD 約 US$10,760／rack；Rubin Ultra 冷板內容值較 VR200 再增約 58%，QD 再增約 103%，均需以實際 BOM、認證與供應商份額驗證。
- 供應鏈受惠排序：冷板觀察 [[3017_奇鋐（市）]]；QD 觀察 [[6805_富世達（市）]]、[[3533_嘉澤（市）]]；系統整合觀察 [[2317_鴻海（市）]]、[[SMCI.US(supermicro)]]。來源：[[報告_GHHK_AI伺服器散熱_20260901]]。
- BYD Electronics 的 VR200 冷板布局另見 [[00285.HK(byd_electronic)]]；出貨時程來自 GF Securities 轉述，尚待實際認證與出貨確認。

## 2026-09-06 廣閎科補充

[[6693_廣閎科（櫃）]] 位於功率 MOSFET、BLDC／SOC FAN 與 AI 伺服器散熱的交界，法說稱 12V／48V AI chassis fan、5G cooling 已認證量產，伺服器營收占比 2026Q2 升至 47.6%。資料來自 AI 生成法說 memo，需以正式公告、實際出貨與客戶認證追蹤。來源：[[活動_廣閎科_2026Q2法說_20260821]]。

## 2026-09-15 UBS／2026-09-17 Citi 熱管理更新

- UBS 觀察 GB300 約 140kW、VR200 約 250kW，三家 hyperscaler 正評估 400V 架構；液冷、電源分配與機櫃整合的單機價值持續提高。來源：[[報告_UBS_台灣高峰會Day1科技硬體_20260915]]。
- Citi 指出 Rubin 9 月小量出貨、4Q26 放量，ASIC 平台由氣冷轉液冷；冷板需求仍高於供給，奇鋐 2027 年 ASIC 液冷營收占比估逾 50%，雙鴻液冷營收占比估由 61% 升至 70%。上述為券商 estimate，仍待認證與出貨驗證。來源：[[報告_Citi_AI伺服器散熱_20260917]]。
