...
| 解法 | 主要作用 | 受惠環節 | 關鍵限制 |
|---|---|---|---|
| NAND／KV Cache offload | 以較低成本增加容量，承接較冷的 KV Cache | NAND die、eSSD、控制器、buffer DRAM | 延遲、IOPS、耐久度、資料搬移與命中率 |
| CXL memory pooling | 提高分散 DRAM 的可調度性與利用率 | CXL switch／controller、memory expander、系統軟體 | 協定世代、一致性、延遲、拓撲與軟體調度 |
| NVLink／NVSwitch＋光學 fabric | 擴大 scale-up domain，嘗試把高速資源存取延伸至跨 rack | 交換器、NPO／CPO、光引擎、雷射、FAU、光纖 | 封閉生態、光電轉換、故障域、功耗與 TCO |

[[memo_FMS2026_CXL記憶體池與光互連_日期不詳]]（日期不詳，FMS 2026 觀察）據此提出「NAND＋光互連」可能同受惠於 DRAM 稀缺。這是可追蹤 thesis：CXL 與 NVLink 並非同一協定，遠端可達也不必然等同透明 memory pooling；Marvell、NVIDIA 與 SK hynix 的產品敘述仍需原始簡報與官方規格查證。詳見 [[分析_FMS2026_CXL記憶體池與光互連受惠邏輯]]。
...
- [[memo_FMS2026_CXL記憶體池與光互連_日期不詳]] — 使用者 FMS 2026 產業觀察，日期不詳（NAND、CXL memory pooling 與跨 rack 光互連 thesis；未查證）
